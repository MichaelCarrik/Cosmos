...
#ifndef COSMOS_VULTUREFAST_H
#define COSMOS_VULTUREFAST_H
...
namespace Cosmos {
    namespace Policy {
        class VultureFast : public IFuturePolicy {
        private:
            int m_length{1000};
            double m_alpha{0.7};
            double m_mark{2.0};
            ...
            int m_todayKlineCounts{0};

        public:
            VultureFast(std::string const &policyName, std::string const &engineName,
            ...
            void _initVulture() {
                ...
                for (auto i = 0; i < m_underlyKseries->m_seriesIndex; i++) {
                    ...
                    if (m_tradingDay == m_underlyKseries->m_KDataVecs[i]->m_tradingDay) {
                        m_todayKlineCounts += 1;
                    }
                }
            ...
            void _updateVultureSignal(const KData::KData *lastUnderlyBar) {
                ...
                if (m_tradingDay == lastUnderlyBar->m_tradingDay) {
                    m_todayKlineCounts += 1;
                }
            }
                ...
                if (strcmp(pMD->instrumentID.data(), m_underlyInstrument.data()) == 0) {
                    if (m_lastUnderlyBarIndex == 0 and m_underlyKseries->m_seriesIndex > m_lastUnderlyBarIndex) {
                        ...
                        if (m_tradeNum < 10 && m_trendSignal.marketPosition == 0) {
                            if (lastBar->m_close >= m_upBand && lastBar->m_close > lastBar->m_open ) //long open
                            {
                                ++m_tradeNum;
                            ...
                            } else if (lastBar->m_close <= m_downBand && lastBar->m_close < lastBar->m_open) //short open
                            {
                                ++m_tradeNum;
                                m_trendSignal.marketPosition = -1;
                                m_targetSignal.targetPosMaps[m_underlyInstrument] = -m_lots;
            ...
            virtual void writePolicyLog(const KData::KData *lastUnderlyKB, const Types::MarketData *pMD) override {
                m_configLog->info("{}, {}, {}, {}, {:03d}, close={:.3f}({:.3f}, {:.3f}), open={:.3f}, band={:.3f}, "
                    "upBand={:.3f}, downBand={:.3f}, TDKCounts={}, mktPos={}, sgnPrice={:.3f},  tgtPos={}, lots={}",
                    ...
                    m_downBand,m_todayKlineCounts, m_trendSignal.marketPosition, m_trendSignal.signalPrice,
                    m_targetSignal.targetPosMaps[m_underlyInstrument], m_lots
                );
                m_configLog->flush();
            };
