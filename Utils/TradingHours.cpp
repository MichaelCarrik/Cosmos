    ...
    namespace Utils {
      ...
      //  std::map<Types::Instrument_t, TradingSession> TradingHours::m_instrumentTradingSession;
        ...
        void TradingHours::loadTradingHourTypes(boost::property_tree::ptree &pt,
            ...
            for (auto tradingHour: boost::make_iterator_range(
                     pt.get_child("TradingHour.TradingHourTypes").equal_range("TradingHourType"))) {
        ...
        void TradingHours::loadConfig(std::string &config) {
            ...
            for (auto exchange: boost::make_iterator_range(
                     pt.get_child("TradingHour").get_child("Exchanges").equal_range("Exchange"))) {
                ...
                for (auto product: boost::make_iterator_range(products_pt)) {
                    ...
                    if (itr_th != tradingHourTypesMap.end()) {
                        ...
                        auto resultMap = new std::unordered_map<int, int>();
                        TradingHours::calTradeSection(itr_th->second, resultMap);
                        TradingHours::m_productPsTimeToSession[ptchars] = resultMap;
                    } else {
        ...
        void TradingHours::calTradeSection(TradingSession const &tradingSession,
                                           std::unordered_map<int, int> *resultMap) {
            int numb = 1;
            for (auto &sector: tradingSession.tradingVec) {
                int initTime = std::floor(sector.beginTime / 1800.0) * 1800;
                while (sector.beginTime <= initTime && initTime < sector.endTime) {
                    (*resultMap)[initTime] = numb;
                    numb++;
                    initTime += 1800;
                }
            }
        ...
        int TradingHours::getPsTimeToNumb(Types::Product_t product, int psTime , int& sectorSize) {
            int sectorTime = std::floor(psTime / 1800.0) * 1800;
