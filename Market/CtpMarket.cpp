...
namespace Cosmos{
    namespace Market{
        int CtpMarket::start(std::vector<Types::MarketData *> const& initMarketDataVector){
            ...
            m_ctpMarketConnection.username = pt.get_child("ThostUser2.ConnectConfig.Username").get<std::string>("<xmlattr>.value");
            m_ctpMarketConnection.password = pt.get_child("ThostUser2.ConnectConfig.Password").get<std::string>("<xmlattr>.value");
            m_ctpMarketConnection.marketFront = pt.get_child("ThostUser2.ConnectConfig.MarketFront").get<std::string>("<xmlattr>.address");
            m_ctpMarketConnection.brokerId = pt.get_child("ThostUser2.ConnectConfig.BrokerID").get<std::string>("<xmlattr>.name");
         ...
         //   m_pMdApi->SubscribePublicTopic(THOST_TERT_RESTART);
            ...
            std::copy(std::begin(m_ctpMarketConnection.marketFront), std::end(m_ctpMarketConnection.marketFront), std::begin(marketFront));
            ...
            for(auto& initMarket : initMarketDataVector){
                ...
                if(itr != m_subScribeInstruments.end()){
                    if( Utils::FTTrait::FT_TRADING !=  Utils::TradingHours::getProductTrait(initMarket->instrumentID, initMarket->psSecond)){
                        continue;
                    }
                   ...
                   //     fprintf(stderr, "initMarket %s %s\n", initMarket->instrumentID.data(), initMarket->updateTime.data());
    ...
    //            std::copy(std::begin(pDepthMarketData->TradingDay), std::begin(pDepthMarketData->TradingDay) + marketData->tradingDay.size(), std::begin(marketData->tradingDay));
                ...
                marketData->isInit = 0;
                if( Utils::FTTrait::FT_TRADING !=  Utils::TradingHours::getProductTrait(marketData->instrumentID, marketData->psSecond)){
                    return;
                }

//                if(Utils::isAbtrgInstrument(marketData->instrumentID) == false){
//                    for(auto period : PairsTrading::Types::m_kperoidVec) {
...
//            if(Utils::isAbtrgInstrument(instrument) == true){
//                onSubScribeQuote.m_kSeriesMap = nullptr;
//            }else{
